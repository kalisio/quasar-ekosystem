...
export const palette = {
  ...
  resolve (color, defaultColor) {
    if (is.empty(color)) return defaultColor
  ...
  findClosest (color) {
    assert.that(color, commonColor.is, 'color must be a valid color')
    let minDistance = Number.MAX_VALUE
    let closestColor = null
    for (const key in QUASAR_PALETTE) {
      const d = chroma.deltaE(color, QUASAR_PALETTE[key])
      if (d < minDistance) {
        minDistance = d
        closestColor = key
      }
